const FAMILIA_BASE = `
  SELECT
    f.id_familia, f.nombre_familia, f.residencia, f.papa_id, f.mama_id,
    f.direccion, f.foto_portada_url, f.foto_perfil_url, f.descripcion,
    (p.nombre + ' ' + p.apellido) AS papa_nombre,
    (m.nombre + ' ' + m.apellido) AS mama_nombre,
    p.num_empleado AS papa_num_empleado, m.num_empleado AS mama_num_empleado,
    p.telefono AS papa_telefono, m.telefono AS mama_telefono,
    p.foto_perfil AS papa_foto_perfil_url, m.foto_perfil AS mama_foto_perfil_url
...
const FAMILIA_QUERIES = {
  list:         `${FAMILIA_BASE} WHERE f.activo = 1 ORDER BY f.nombre_familia`,
  byId:         `${FAMILIA_BASE} WHERE f.id_familia = @id_familia AND f.activo = 1`,
  byName:       `${FAMILIA_BASE} WHERE f.nombre_familia LIKE @like ORDER BY f.nombre_familia`,
  byIdent: `
    ${FAMILIA_BASE}
    JOIN EDI.Miembros_Familia mf ON mf.id_familia = f.id_familia
    JOIN EDI.Usuarios u ON u.id_usuario = mf.id_usuario
    WHERE (u.matricula = @ident OR u.num_empleado = @ident) AND f.activo = 1
  `,
  softDelete:   `UPDATE EDI.Familias_EDI SET activo = 0 WHERE id_familia = @id_familia`,
  update: `
    UPDATE EDI.Familias_EDI SET
  ...
  `,
  reporteCompleto: `
    SELECT f.id_familia, f.nombre_familia, f.residencia,
           (p.nombre + ' ' + p.apellido) AS papa_nombre,
           (m.nombre + ' ' + m.apellido) AS mama_nombre,
           miembros.id_usuario,
  ...
  `,
  listAvailable: `
    SELECT f.id_familia, f.nombre_familia, f.foto_portada_url AS portada,
           f.residencia, f.descripcion,
...
module.exports = { FAMILIA_QUERIES, FAMILIA_BASE };
