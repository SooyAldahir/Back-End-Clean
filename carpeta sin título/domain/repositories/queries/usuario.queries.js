const USUARIO_QUERIES = {
  byLogin: `
    SELECT TOP 1
      u.id_usuario, u.nombre, u.apellido, u.correo, u.contrasena, u.foto_perfil,
      u.tipo_usuario, u.matricula, u.num_empleado, u.id_rol, u.estado, u.activo,
      u.created_at, u.updated_at, u.telefono, u.residencia, u.direccion,
      u.fecha_nacimiento, u.carrera, u.session_token, r.nombre_rol,
      (SELECT TOP 1 mf.id_familia FROM EDI.Miembros_Familia mf
       WHERE mf.id_usuario = u.id_usuario AND mf.activo = 1
       ORDER BY mf.id_miembro DESC) AS id_familia,
      (SELECT TOP 1 f.nombre_familia FROM EDI.Miembros_Familia mf
  ...
  insert: `
    DECLARE @out TABLE (
      id_usuario INT, nombre NVARCHAR(100), apellido NVARCHAR(100),
      tipo_usuario NVARCHAR(50), matricula INT, num_empleado INT,
      correo NVARCHAR(255), estado NVARCHAR(50), created_at DATETIME,
      updated_at DATETIME, telefono NVARCHAR(20), residencia NVARCHAR(50),
      direccion NVARCHAR(200), fecha_nacimiento DATE, carrera NVARCHAR(120)
    );
    INSERT INTO EDI.Usuarios
      (nombre, apellido, correo, contrasena, foto_perfil, tipo_usuario,
       matricula, num_empleado, id_rol, telefono, residencia, direccion,
       fecha_nacimiento, carrera)
    OUTPUT INSERTED.id_usuario, INSERTED.nombre, INSERTED.apellido, INSERTED.tipo_usuario,
           INSERTED.matricula, INSERTED.num_empleado, INSERTED.correo, INSERTED.estado,
           INSERTED.created_at, INSERTED.updated_at, INSERTED.telefono, INSERTED.residencia,
           INSERTED.direccion, INSERTED.fecha_nacimiento, INSERTED.carrera
    INTO @out
    VALUES (@nombre, @apellido, @correo, @contrasena, @foto_perfil, @tipo_usuario,
            @matricula, @num_empleado, @id_rol, @telefono, @residencia, @direccion,
            @fecha_nacimiento, @carrera);
    SELECT * FROM @out;
  `,
  ...
  updateBasic: `
    DECLARE @out TABLE (
      id_usuario INT, nombre NVARCHAR(100), apellido NVARCHAR(100),
      tipo_usuario NVARCHAR(50), matricula INT, num_empleado INT,
      correo NVARCHAR(255), estado NVARCHAR(50), updated_at DATETIME,
      telefono NVARCHAR(20), residencia NVARCHAR(50), direccion NVARCHAR(200),
      fecha_nacimiento DATE, carrera NVARCHAR(120)
    );
    UPDATE EDI.Usuarios SET
    ...
    OUTPUT INSERTED.id_usuario, INSERTED.nombre, INSERTED.apellido, INSERTED.tipo_usuario,
           INSERTED.matricula, INSERTED.num_empleado, INSERTED.correo, INSERTED.estado,
           INSERTED.updated_at, INSERTED.telefono, INSERTED.residencia, INSERTED.direccion,
           INSERTED.fecha_nacimiento, INSERTED.carrera
    INTO @out
  ...
  list: `
    SELECT u.id_usuario, u.nombre, u.apellido, u.correo, u.tipo_usuario,
           u.matricula, u.num_empleado, u.estado, u.activo, r.nombre_rol
    FROM EDI.Usuarios u
    JOIN EDI.Roles r ON r.id_rol = u.id_rol
  `,

  byId: `
    SELECT TOP 1 u.*, r.nombre_rol, f.nombre_familia, f.id_familia,
  ...
  birthdaysToday: `
    SELECT id_usuario, nombre, apellido, url_foto_perfil, fecha_nacimiento
    FROM EDI.Usuarios
  ...
  integrarComunidadPadres: `
    ...
    BEGIN
      ...
      IF @IdSala IS NOT NULL AND NOT EXISTS (
        SELECT 1 FROM EDI.Chat_Participantes WHERE id_sala = @IdSala AND id_usuario = @idUsuario
      )
      BEGIN
...
module.exports = { USUARIO_QUERIES };
